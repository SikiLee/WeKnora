...
func (r *feedbackRepository) ApplyMessageFeedback(
	...
	err := r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		maxRevision := int64(0)
		if hasExisting {
			maxRevision = existing.FeedbackRevision
		}
		for _, chunk := range chunks {
			if chunk.FeedbackResetRevision > maxRevision {
				maxRevision = chunk.FeedbackResetRevision
			}
		}
		...
		for _, chunk := range chunks {
			activeEverywhere = activeEverywhere && existing.FeedbackRevision > chunk.FeedbackResetRevision
		}
		...
		case types.FeedbackTypeLike, types.FeedbackTypeDislike:
			...
			existing.FeedbackRevision = maxRevision + 1
...
func recomputeChunks(
	...
	for _, key := range keys {
		...
		if err := tx.Table("message_feedbacks AS mf").
			...
			Joins("JOIN message_chunk_references AS mcr ON mcr.message_tenant_id = mf.tenant_id AND mcr.message_id = mf.message_id").
			Joins("JOIN messages AS m ON m.id = mf.message_id AND m.deleted_at IS NULL").
			Where("mcr.chunk_tenant_id = ? AND mcr.chunk_id = ? AND mf.feedback_revision > ?",
				key.tenantID, key.chunkID, chunk.FeedbackResetRevision).
			Group("mf.feedback_type").Scan(&rows).Error; err != nil {
			return err
		}
		var likes, dislikes int64
		for _, row := range rows {
			if row.FeedbackType == types.FeedbackTypeLike {
				likes = row.Count
			} else if row.FeedbackType == types.FeedbackTypeDislike {
				dislikes = row.Count
			}
		}
...
func (r *feedbackRepository) ResetChunkFeedback(
	...
	return r.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Table("message_feedbacks AS mf").
			...
			Select("COALESCE(MAX(mf.feedback_revision), 0)").Scan(&maxRevision).Error; err != nil {
			return err
		}
		if maxRevision < chunk.FeedbackResetRevision {
			maxRevision = chunk.FeedbackResetRevision
		}
		oldWeight := chunk.RecallWeight
		if oldWeight <= 0 {
		...
		if err := tx.Model(&types.Chunk{}).
			...
			Updates(map[string]interface{}{
				...
				"positive_rate":           nil,
				"recall_weight":           1.0,
				"feedback_reset_revision": maxRevision,
			}).Error; err != nil {
			return err
		}
...
func (r *feedbackRepository) GetChunkFeedbackDetails(
	...
	if err := r.db.WithContext(ctx).Table("message_feedbacks AS mf").
		Select("mf.reason_code, COUNT(*) AS count").
		Joins("JOIN message_chunk_references AS mcr ON mcr.message_tenant_id = mf.tenant_id AND mcr.message_id = mf.message_id").
		Joins("JOIN messages AS m ON m.id = mf.message_id AND m.deleted_at IS NULL").
		Where("mcr.chunk_tenant_id = ? AND mcr.chunk_id = ? AND mf.feedback_type = ? AND mf.feedback_revision > ?",
			tenantID, chunkID, types.FeedbackTypeDislike, chunk.FeedbackResetRevision).
		Where("mf.reason_code IS NOT NULL").
		Group("mf.reason_code").Scan(&reasons).Error; err != nil {
		return nil, err
	}
