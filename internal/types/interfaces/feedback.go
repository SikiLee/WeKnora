...
type FeedbackRepository interface {
	...
	ListChunkFeedback(ctx context.Context, tenantID uint64, kbID string, query *types.ChunkFeedbackListQuery, cfg *types.ChunkFeedbackConfig) ([]*types.ChunkFeedbackListItem, int64, error)
	GetChunkFeedbackDetail(ctx context.Context, tenantID uint64, kbID, chunkID string) (*types.ChunkFeedbackDetail, error)
	ListChunkFeedbackWeightLogs(ctx context.Context, tenantID uint64, kbID, chunkID string, page *types.Pagination) ([]*types.ChunkFeedbackWeightLogItem, int64, error)
	ResetChunkFeedback(ctx context.Context, tenantID uint64, kbID, chunkID, reason string, cfg *types.ChunkFeedbackConfig) (*types.ChunkFeedbackDetail, error)
}
...
type FeedbackService interface {
	...
	ListChunkFeedbackWeightLogs(ctx context.Context, kbID, chunkID string, page *types.Pagination) (*types.PageResult, error)
	ResetChunkFeedback(ctx context.Context, kbID, chunkID string, input *types.ChunkFeedbackResetInput) (*types.ChunkFeedbackDetail, error)
}
